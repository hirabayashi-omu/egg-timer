    ...
    if (saved) {
        try {
            ...
            if (data.version && data.root) {
                ...
                state.columnLabels = data.columnLabels || {};
...
function undo() {
    ...
    state.columnLabels = previous.columnLabels || {};
...
function redo() {
    ...
    state.columnLabels = next.columnLabels || {};
...
function updateUndoRedoButtons() {
    ...
    if (redoBtn) {
        ...
        redoBtn.style.opacity = state.future.length === 0 ? '0.5' : '1';
    }
}
...
function init() {
    ...
    if (addImgBtn) {
        addImgBtn.onclick = () => {
            ...
            const centerX = (rect.width / 2 - state.pan.x) / state.scale;
            const centerY = (rect.height / 2 - state.pan.y) / state.scale;
    ...
    if (imgInput) {
        imgInput.onchange = (e) => {
            ...
            if (file && state.selectedImageId) {
                const reader = new FileReader();
                reader.onload = (evt) => {
                    const imgIdx = state.images.findIndex(img => img.id === state.selectedImageId);
                    if (imgIdx !== -1) {
                        saveState();
                        state.images[imgIdx].src = evt.target.result;
                        render();
                    }
                };
...
function renderImages() {
    ...
    layer.style.transform = `translate(${state.pan.x}px, ${state.pan.y}px) scale(${state.scale})`;

    state.images.forEach(img => {
        const frame = document.createElement('div');
        frame.className = 'image-frame';
        if (state.selectedImageId === img.id) frame.classList.add('selected');
        ...
        if (state.selectedImageId === img.id) {
            ...
            resizer.onmousedown = (e) => {
                e.stopPropagation();
                state.imageResize.active = true;
                state.imageResize.id = img.id;
                state.imageResize.startX = e.clientX;
                state.imageResize.startY = e.clientY;
                state.imageResize.startW = img.width;
                state.imageResize.startH = img.height;
            };
        ...
        frame.onmousedown = (e) => {
            ...
            state.imageDrag.id = img.id;
            state.imageDrag.lastX = e.clientX;
            state.imageDrag.lastY = e.clientY;
        };
                    ...
                    for (const clipboardItem of clipboardItems) {
                        for (const type of clipboardItem.types) {
                            if (type.startsWith('image/')) {
                                ...
                                const reader = new FileReader();
                                reader.onload = (evt) => {
                                    const imgIdx = state.images.findIndex(i => i.id === img.id);
                                    if (imgIdx !== -1) {
                                        saveState();
                                        state.images[imgIdx].src = evt.target.result;
                                        render();
...
function addSibling(id) {
    if (id === state.root.id) return; // Can't add sibling to root
...
function deleteNode(id) {
    if (id === state.root.id) return; // Can't delete root
    saveState();
    const parent = findParent(id);
    if (!parent) return;
        ...
        COLOR_PALETTE.forEach(color => {
            ...
            swatch.onclick = (e) => {
                ...
                if (state.contextMenu.nodeId) {
                    const node = findNode(state.contextMenu.nodeId);
                    if (node) {
                        saveState();
                        node.color = color;
                        render();
...
function updateLayout() {
    ...
    findMaxWidths(root);
    state.columnMaxWidths = columnMaxWidths; // Save for centering in positionNodes
...
function positionNodes(node, startY) {
    ...
    node.x = state.columnOffsets[col] || (col * (CONFIG.nodeWidth + 100));
...
// Rendering
function updateView() {
    const cssTransform = `translate(${state.pan.x}px, ${state.pan.y}px) scale(${state.scale})`;
    const svgTransform = `translate(${state.pan.x}, ${state.pan.y}) scale(${state.scale})`;
...
function render() {
    ...
    renderNodeRecursive(state.root);

    // 2. Measure actual dimensions from DOM
    measureNodesRecursive(state.root);

    // 3. Update Layout with actual sizes
    updateLayout();

    // 4. Update Node Positions in DOM
    applyNodePositionsRecursive(state.root);
    ...
    if (state.columnOffsets) {
        const cols = Object.keys(state.columnOffsets).map(Number).sort((a, b) => a - b);
        const gapBetween = 100;

        cols.forEach((col, i) => {
            if (i > 0) {
                const prevCol = cols[i - 1];
                const x = state.columnOffsets[col] - (gapBetween / 2);
                ...
                // Generation Label
                const labelY = -state.root.subtreeHeight / 2 - 60;

                if (state.editingLabelColumn === col) {
                    ...
                    input.value = (state.columnLabels && state.columnLabels[col]) || `GEN ${col}`;
                    ...
                    const saveLabel = () => {
                        if (!state.columnLabels) state.columnLabels = {};
                        state.columnLabels[col] = input.value;
                        state.editingLabelColumn = null;
                        render();
                    };
                    ...
                    label.textContent = (state.columnLabels && state.columnLabels[col]) || `GEN ${col}`;

                    // Double click to edit
                    label.addEventListener('dblclick', (e) => {
                        e.stopPropagation();
                        e.preventDefault();
                        state.editingLabelColumn = col;
                        render();
                    });
    ...
    renderConnectionsRecursive(state.root, bgG, fgG);

    // Extra Relations
    renderRelationsRecursive(state.root, bgG, fgG);
    // Connection Drag Line
    if (state.connectionDrag.active) {
        const sourceNode = findNode(state.connectionDrag.sourceNodeId);
        if (sourceNode) {
            ...
            if (el) {
                const offset = 6;
                startX = state.connectionDrag.type === 'out'
                    ? sourceNode.x + el.offsetWidth + offset
                    : sourceNode.x - offset;
            }

            const rect = canvasContainer.getBoundingClientRect();
            const worldMouseX = (state.connectionDrag.currentPos.x - rect.left - state.pan.x) / state.scale;
            const worldMouseY = (state.connectionDrag.currentPos.y - rect.top - state.pan.y) / state.scale;
...
function renderNodeRecursive(node) {
    ...
    if (node.id === state.root.id) classes.push('root');
    if ((node.id === state.dropTargetId && state.isNodeDragging) ||
        (node.id === state.dropTargetId && state.connectionDrag.active)) {
        classes.push('drop-target');
    }
    if (node.id === state.draggedNodeId && state.isNodeDragging) classes.push('dragging');
    ...
    if (state.isNodeDragging && state.dropTargetId === node.id && state.dropTargetType === 'sibling') {
        ...
        const draggedParent = findParent(state.draggedNodeId);
        if (parent && draggedParent && parent.id === draggedParent.id) {
            const children = parent.children;
            const targetIdx = children.findIndex(c => c.id === node.id);
            const draggedIdx = children.findIndex(c => c.id === state.draggedNodeId);

            // Logic to match calculateSubtreeHeight/mousemove logic:
            // state.dropTargetIndex is the splice target.
            if (state.dropTargetIndex <= targetIdx) {
    ...
    el.onmousedown = (e) => {
        ...
        if (e.target.classList.contains('handle')) {
            ...
            state.connectionDrag.sourceNodeId = node.id;
            state.connectionDrag.type = type;
            state.connectionDrag.currentPos = { x: e.clientX, y: e.clientY };
        ...
        state.potentialDragNodeId = node.id;
    };
...
function drawConnection(source, target, bgG, fgG, isRelation, index = 0, archPreference = 'auto', customArchWidth = null, customArchHeight = null) {
    ...
    if (parentEl && targetEl) {
        const isSelected = state.selectedConnection && state.selectedConnection.sourceId === source.id && state.selectedConnection.targetId === target.id;
        ...
        hitPath.addEventListener('mousedown', (e) => {
            ...
            state.selectedConnection = { sourceId: source.id, targetId: target.id };
            render();
        });
        ...
        handleGroup.addEventListener('mousedown', (e) => {
            if (e.button === 0) {
                e.stopPropagation();
                state.archDrag.active = true;
                state.archDrag.dragged = false;
                state.archDrag.sourceId = source.id;
                state.archDrag.targetId = target.id;
                state.archDrag.isRelation = isRelation;
                state.archDrag.startX = e.clientX;
                state.archDrag.startY = e.clientY;

                // Cache elements to prevent lag from querySelector in mousemove
                state.archDrag.cachedPath = targetGroup.querySelector(`path.connection[data-source="${source.id}"][data-target="${target.id}"]`);
                state.archDrag.cachedHandleGroup = handleGroup;

                // Cache start values
                state.archDrag.startArchWidth = customArchWidth !== null ? customArchWidth : (isSelfLoop ? 150 : 70);
                if (colStep > 0 && !isSelfLoop) state.archDrag.startArchWidth = customArchWidth !== null ? customArchWidth : 60;

                state.archDrag.startArchHeight = customArchHeight !== null ? customArchHeight : (isSelfLoop ? 100 : 60);
                if (colStep > 0 && !isSelfLoop) state.archDrag.startArchHeight = customArchHeight !== null ? customArchHeight : (80 + (absStep > 1 ? (absStep - 1) * 30 : 0));
        ...
        handleGroup.onclick = (e) => {
            if (!state.archDrag.active && !state.archDrag.dragged) {
                e.stopPropagation();
...
function selectNode(id) {
    ...
    if (state.selectedNodeId) {
        const prevEl = nodesLayer.querySelector(`[data-id="${state.selectedNodeId}"]`);
    ...
    // Add class to new
    if (state.selectedNodeId) {
        const nextEl = nodesLayer.querySelector(`[data-id="${state.selectedNodeId}"]`);
...
function enterEditMode(id) {
    ...
    resizerY.onmousedown = (e) => {
        e.preventDefault(); e.stopPropagation();
        state.nodeResize.active = true;
        state.nodeResize.nodeId = id;
        state.nodeResize.axis = 'v';
        state.nodeResize.startY = e.clientY;
        state.nodeResize.startSize = el.offsetHeight;
    ...
    resizerX.onmousedown = (e) => {
        ...
        state.nodeResize.active = true;
        state.nodeResize.nodeId = id;
        state.nodeResize.axis = 'h';
        state.nodeResize.startX = e.clientX;
        state.nodeResize.startSize = el.offsetWidth;
...
function setupEventListeners() {
    ...
    document.addEventListener('keydown', (e) => {
        ...
        switch (e.key) {
            case 'Tab':
                e.preventDefault();
                if (state.selectedNodeId) addNode(state.selectedNodeId);
                break;
            case 'Enter':
                e.preventDefault();
                if (state.selectedNodeId) {
                    if (state.selectedNodeId === state.root.id) {
            ...
            case 'Backspace':
                ...
                break;
            case 'ArrowUp':
                if (e.altKey && state.selectedNodeId) {
                    e.preventDefault();
                    moveNodeUp(state.selectedNodeId);
                }
                break;
            case 'ArrowDown':
                if (e.altKey && state.selectedNodeId) {
                    e.preventDefault();
                    moveNodeDown(state.selectedNodeId);
                }
                break;
        }
    });
    ...
    window.addEventListener('mousemove', (e) => {
        ...
        if (state.archDrag.active) {
            const source = findNode(state.archDrag.sourceId);
            const target = findNode(state.archDrag.targetId);
            if (!source || !target) return;

            if (!state.archDrag.dragged) {
                if (Math.hypot(e.clientX - state.archDrag.startX, e.clientY - state.archDrag.startY) > 5) state.archDrag.dragged = true;
            }

            const rect = canvasContainer.getBoundingClientRect();
            // Current mouse in world space
            const worldMouseX = (e.clientX - rect.left - state.pan.x) / state.scale;
            const worldMouseY = (e.clientY - rect.top - state.pan.y) / state.scale;
            // Start mouse in world space
            const startWorldX = (state.archDrag.startX - rect.left - state.pan.x) / state.scale;
            const startWorldY = (state.archDrag.startY - rect.top - state.pan.y) / state.scale;
            ...
            let newW = state.archDrag.startArchWidth + (worldMouseX - startWorldX);
            ...
            const path = state.archDrag.cachedPath;
            const hGroup = state.archDrag.cachedHandleGroup;
            if (path || hGroup) {
        ...
        if (state.connectionDrag.active) {
            ...
            if (state.dropTargetId !== newTargetId) {
                // Update classes manually to avoid re-render
                if (state.dropTargetId) {
                    const oldEl = nodesLayer.querySelector(`[data-id="${state.dropTargetId}"]`);
                    if (oldEl) oldEl.classList.remove('drop-target');
                }
                state.dropTargetId = newTargetId;
                if (state.dropTargetId) {
                    const newEl = nodesLayer.querySelector(`[data-id="${state.dropTargetId}"]`);
        ...
        // Node Dragging
        if (state.potentialDragNodeId && !state.isNodeDragging) {
            const dx = e.clientX - state.dragStartPos.x;
            const dy = e.clientY - state.dragStartPos.y;
            if (Math.hypot(dx, dy) > 5) { // Threshold
                if (state.potentialDragNodeId !== state.root.id) {
                    state.isNodeDragging = true;
                    state.draggedNodeId = state.potentialDragNodeId;
                    canvasContainer.style.cursor = 'move';
                    render(); // Apply classes
                } else {
                    state.potentialDragNodeId = null; // Can't drag root
                }
            }
        }
        ...
        if (state.isNodeDragging) {
            // Hit Test for Target
            const nodes = document.elementsFromPoint(e.clientX, e.clientY);
            const targetEl = nodes.find(el => el.classList.contains('node') && el.dataset.id !== state.draggedNodeId);
            const newTargetId = targetEl ? targetEl.dataset.id : null;
            ...
            if (state.dropTargetId !== newTargetId || state.dropTargetIndex !== newTargetIndex) {
                state.dropTargetId = newTargetId;
                state.dropTargetType = newTargetType;
                state.dropTargetIndex = newTargetIndex;
        ...
        // Image Drag
        if (state.imageDrag.active) {
            const dx = e.clientX - state.imageDrag.lastX;
            const dy = e.clientY - state.imageDrag.lastY;

            const img = state.images.find(img => img.id === state.imageDrag.id);
            if (img) {
                // We update position. 
        ...
        // Image Resize
        if (state.imageResize.active) {
            const idx = state.images.findIndex(img => img.id === state.imageResize.id);
            if (idx !== -1) {
                const dx = e.clientX - state.imageResize.startX;
                const dy = e.clientY - state.imageResize.startY;

                const newW = Math.max(50, state.imageResize.startW + (dx / state.scale));
                const newH = Math.max(50, state.imageResize.startH + (dy / state.scale));

                state.images[idx].width = newW;
                state.images[idx].height = newH;
                renderImages();
            }
            return;
        }
        ...
        // Node Resizing
        if (state.nodeResize.active) {
            const node = findNode(state.nodeResize.nodeId);
            if (!node) return;

            if (state.nodeResize.axis === 'v') {
                const dy = e.clientY - state.nodeResize.startY;
                node.customHeight = Math.max(CONFIG.nodeHeight, state.nodeResize.startSize + (dy / state.scale));
            } else {
                const dx = e.clientX - state.nodeResize.startX;
                node.customWidth = Math.max(CONFIG.nodeWidth, state.nodeResize.startSize + (dx / state.scale));
            }
            render();
        }
    });
    ...
    window.addEventListener('mouseup', (e) => {
        ...
        if (state.imageDrag.active) {
            ...
            state.imageDrag.id = null;
            persistState(); // Save after move
            return;
        }

        if (state.imageResize.active) {
            state.imageResize.active = false;
            state.imageResize.id = null;
            persistState(); // Save after resize
            return;
        }

        if (state.archDrag.active) {
            state.archDrag.active = false;
            // Clear dragged flag after a small delay to prevent immediate click
            setTimeout(() => { state.archDrag.dragged = false; }, 200);
            canvasContainer.style.cursor = 'grab';
            return;
        }
        ...
        // End Connection Drag
        if (state.connectionDrag.active) {
            const sourceId = state.connectionDrag.sourceNodeId;
            const targetId = state.dropTargetId;
            const type = state.connectionDrag.type;
            ...
            if (state.dropTargetId) {
                const el = nodesLayer.querySelector(`[data-id="${state.dropTargetId}"]`);
            ...
            state.connectionDrag.sourceNodeId = null;
        ...
        if (state.isNodeDragging) {
            if (state.draggedNodeId && state.dropTargetId) {
                if (state.dropTargetType === 'sibling') {
                    // Reorder within parent
                    saveState();
                    const parent = findParent(state.draggedNodeId);
                    const oldIdx = parent.children.findIndex(c => c.id === state.draggedNodeId);
                    const nodeRecord = parent.children.splice(oldIdx, 1)[0];

                    let newIdx = state.dropTargetIndex;
                    if (oldIdx < newIdx) newIdx--; // Adjust for shift after removal
                    parent.children.splice(newIdx, 0, nodeRecord);

                    updateLayout();
                } else if (e.shiftKey) {
                    toggleRelation(state.draggedNodeId, state.dropTargetId);
                } else {
                    moveNodeTo(state.draggedNodeId, state.dropTargetId);
                }
            }
            state.isNodeDragging = false;
            state.draggedNodeId = null;
            state.dropTargetId = null;
            state.dropTargetType = null;
            state.dropTargetIndex = -1;
            state.potentialDragNodeId = null;
            canvasContainer.style.cursor = 'grab';
            render();
            return;
        }
        ...
        if (state.potentialDragNodeId) {
            selectNode(state.potentialDragNodeId);
            state.potentialDragNodeId = null;
        }
    ...
    document.getElementById('add-child-btn').onclick = () => {
        state.selectedNodeId ? addNode(state.selectedNodeId) : showToast('Select a node.', 'error');
    };
    document.getElementById('add-sibling-btn').onclick = () => {
        if (state.selectedNodeId) {
            state.selectedNodeId === state.root.id ?
                (addNode(state.selectedNodeId), showToast('Root cannot have siblings. Added child instead.')) :
                addSibling(state.selectedNodeId);
        } else showToast('Select a node.', 'error');
    };
    document.getElementById('delete-btn').onclick = () => {
        if (state.selectedNodeId) {
            state.selectedNodeId === state.root.id ? showToast('Cannot delete root.', 'error') : deleteNode(state.selectedNodeId);
        } else if (state.selectedImageId) {
            deleteImage(state.selectedImageId);
    ...
    document.getElementById('move-up-btn').onclick = () => {
        state.selectedNodeId ? moveNodeUp(state.selectedNodeId) : showToast('Select a node.', 'error');
    };
    document.getElementById('move-down-btn').onclick = () => {
        state.selectedNodeId ? moveNodeDown(state.selectedNodeId) : showToast('Select a node.', 'error');
    };

    document.getElementById('center-btn').onclick = centerView;
    document.getElementById('zoom-in').onclick = () => zoom(1.2);
    document.getElementById('zoom-out').onclick = () => zoom(0.8);

    // Save/Load
    document.getElementById('export-btn').onclick = saveMap;
    document.getElementById('import-btn-trigger').onclick = () => document.getElementById('import-input').click();
    document.getElementById('import-input').onchange = loadMap;
    document.getElementById('export-pdf-btn').onclick = () => {
        // Manually handle print sequence for better mobile support
        state.prePrintState = {
            pan: { ...state.pan },
            scale: state.scale
        };
        ...
        setTimeout(() => {
            ...
            // Restore after a delay (enough time for print dialog to capture)
            // Note: Mobile browsers might pause execution, so we restore on interaction or long timeout
            setTimeout(() => {
                if (state.prePrintState) {
                    state.pan = state.prePrintState.pan;
                    state.scale = state.prePrintState.scale;
                    updateView();
                    state.prePrintState = null;
                }
            }, 1000);
        }, 500);
    };
...
function centerView() {
    state.pan.x = window.innerWidth / 4; // Start a bit left so tree expands right
...
function fitToScreen(forPrint = false) {
    ...
    if (forPrint) {
        ...
        state.pan.x = (targetWidth / 2) - (centerX * scale);
        state.pan.y = (targetHeight / 2) - (centerY * scale);
    } else {
        state.pan.x = (targetWidth / 2) - (centerX * scale);
        state.pan.y = (targetHeight / 2) - (centerY * scale);
    }
    ...
    reader.onload = (e) => {
        try {
            ...
            if (loaded.root && loaded.version) {
                ...
                state.columnLabels = loaded.columnLabels || {};

                // Restore view state if available
                if (loaded.pan) {
                    state.pan = loaded.pan;
                }
                if (loaded.scale) {
            ...
            updateLayout();
            render();
            persistState(); // Save loaded state to localStorage
...
function updateDragLine() {
    ...
    // End Point
    const rect = canvasContainer.getBoundingClientRect();
    const mouseRelX = state.connectionDrag.currentPos.x - rect.left;
    const mouseRelY = state.connectionDrag.currentPos.y - rect.top;

    const worldMouseX = (mouseRelX - state.pan.x) / state.scale;
    const worldMouseY = (mouseRelY - state.pan.y) / state.scale;
...
function setupTouchEvents() {
    ...
    document.addEventListener('touchmove', (e) => {
        if (state.isNodeDragging ||
            state.imageDrag.active ||
            state.imageResize.active ||
            state.connectionDrag.active ||
            state.archDrag.active ||
            state.nodeResize.active) {
            e.preventDefault();
